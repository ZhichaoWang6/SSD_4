...
#                 self.paddingtensor(item["hidden_state_early"], max_length)
#                 for item in features
#             ]),
#             "target": torch.cat([
...
# ):
#     """
#     Prefix-match aware distillation loss.
...
#     token_loss = -torch.sum(target_p * out_logp, dim=2)  # [B, L]

#     base_mask = loss_mask.squeeze(-1)  # [B, L]

#     pos = torch.cumsum(base_mask, dim=1) * base_mask

#     weights = torch.ones_like(base_mask)
#     prefix_mask = ((pos > 0) & (pos <= prefix_k)).float()
#     weights = weights + prefix_boost * prefix_mask

#     final_mask = base_mask * weights

#     loss = (token_loss * final_mask).sum() / final_mask.sum().clamp_min(1.0)
#     return loss


# def compute_prefix_match(out_head, target_head, loss_mask):
#     """
#     Prefix: 所有样本平均连续命中 token 数
#     LongPrefix: 只统计长回复样本，过滤 NO REPLY
#     FirstAcc: 第一个 token 命中率
#     """
...
#             # len(t) > 5 近似认为是长回复，过滤 NO REPLY
#             if len(t) > 5:
#                 long_prefix_lens.append(m)

#         avg_prefix_match = sum(prefix_lens) / max(len(prefix_lens), 1)
#         long_prefix_match = sum(long_prefix_lens) / max(len(long_prefix_lens), 1)
#         first_token_acc = first_correct / max(valid_samples, 1)

#     return avg_prefix_match, long_prefix_match, first_token_acc
...
#                 prob_acc = (
#                     torch.sum(data["loss_mask"] * prob_acc_per_token)
#                     / data["loss_mask"].sum().clamp(min=1)
#                 )

#                 avg_prefix_match, long_prefix_match, first_token_acc = compute_prefix_match(
#                     out_head=out_head,
#                     target_head=target_head,
#                     loss_mask=data["loss_mask"],
#                 )
...
#                     print(
#                         f"\nStep: {batch_idx}"
#                         f"\tLR: {optimizer.optimizer.param_groups[0]['lr']:.2e}"
#                         f"\tAccept: {prob_acc.item():.4f}"
#                         f"\tPrefix: {avg_prefix_match:.2f}"
#                         f"\tLongPrefix: {long_prefix_match:.2f}"
#                         f"\tFirstAcc: {first_token_acc:.3f}"
#                         f"\tLoss: {loss.item():.4f}"
#                         f"\tLong_conf: {long_conf_avg:.4f}"
#                     )
...
#                 writer.add_scalar("train/loss", loss.item(), global_step)
#                 writer.add_scalar("train/prob_accept", prob_acc.item(), global_step)
#                 writer.add_scalar("train/prefix_match", avg_prefix_match, global_step)
#                 writer.add_scalar("train/long_prefix_match", long_prefix_match, global_step)
#                 writer.add_scalar("train/first_token_acc", first_token_acc, global_step)
#                 writer.add_scalar("train/accuracy", cc / ct, global_step)
#                 writer.add_scalar("train/confidence", batch_confidence_sum / max(ct, 1), global_step)

#             epoch_loss += loss.item()
#             epoch_accept += prob_acc.item()
#             epoch_prefix += avg_prefix_match
#             epoch_long_prefix += long_prefix_match
#             epoch_first_acc += first_token_acc
#             num_batches += 1
...
#         epoch_loss /= max(num_batches, 1)
#         epoch_accept /= max(num_batches, 1)
#         epoch_prefix /= max(num_batches, 1)
#         epoch_long_prefix /= max(num_batches, 1)
#         epoch_first_acc /= max(num_batches, 1)
...
#                 f"  Acc: {100 * epoch_acc:.2f}%"
#                 f"  Accept: {epoch_accept:.4f}"
#                 f"  Prefix: {epoch_prefix:.2f}"
#                 f"  LongPrefix: {epoch_long_prefix:.2f}"
#                 f"  FirstAcc: {epoch_first_acc:.3f}"
#                 f"  Confidence: {epoch_confidence:.4f}"
#                 f"  Long_conf: {epoch_long_confidence:.4f}"
#             )
...
#                 f"_acc{epoch_acc:.4f}"
#                 f"_accept{epoch_accept:.4f}"
#                 f"_prefix{epoch_prefix:.2f}"
#                 f"_longprefix{epoch_long_prefix:.2f}"
#                 f"_loss{epoch_loss:.4f}"
#             )
...
#                 writer.add_scalar("epoch/accuracy", epoch_acc, epoch)
#                 writer.add_scalar("epoch/accept", epoch_accept, epoch)
#                 writer.add_scalar("epoch/prefix_match", epoch_prefix, epoch)
#                 writer.add_scalar("epoch/long_prefix_match", epoch_long_prefix, epoch)
#                 writer.add_scalar("epoch/first_token_acc", epoch_first_acc, epoch)
#                 writer.add_scalar("epoch/confidence", epoch_confidence, epoch)
#                 writer.add_scalar("epoch/long_reply_confidence", epoch_long_confidence, epoch)

#         if epoch % args.save_freq == 0 or epoch == args.start_epoch + args.num_epochs - 1:
#             accelerator.save_state(
#                 output_dir=os.path.join(args.outdir, "state", f"state_{epoch}")
#             )
